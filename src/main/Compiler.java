...
public class Compiler {
	...
	private Program program() {
		...
		while (token == Symbol.FOR
				|| token == Symbol.WHILE
				|| token == Symbol.IF
				|| token == Symbol.ID // assignStat Ã© iniciado por um identificador
				|| token == Symbol.PRINT
...
//			Expr e_else = expr();
	...
	private StatList statList() {
		...
		while (token == Symbol.FOR
				|| token == Symbol.WHILE
				|| token == Symbol.IF
				|| token == Symbol.ID // assignStat comeÃ§a com um identificador
				|| token == Symbol.PRINT
				|| token == Symbol.PRINT_LINE) {
			s.addStat(stat());
		}
...
//		Var actual_var = new Var(ident);
	...
	//	SimpleExpr ::= Number | â€™(â€™ Expr â€™)â€™ | "!" SimpleExpr| AddOp SimpleExpr | Ident
...
//					this.number.setValue(this.number.getValue() * 10 + (ch - '0'));
	...
	// verifica se o token corrente Ã© aquele esperado (se for avanÃ§a, caso contrÃ¡rio lanÃ§a um erro)
